    ...
    Qx::prv::GraphicsWindow{ parent }
{
    ...
    m_meshQuery = ecsWorld->query_builder<Qx::MeshView>().build();
    ecsCmp->preProgress = [ecsWorld, this, model]()
    {
        // dbg_print_st() << "----------------------------------";
        // dbg_print_st() << "Pre..." ;


        if( m_meshQuery.count() != 0 )
        {
            /// ## If the count is not 0
            /// ## Don't add more entities
            dbg_print_st() << m_meshQuery.count();
            return;
        }
        // else
        //     dbg_print_st() << m_meshQuery.count();
        ...
        model->ready = true;
        /// ## Clear and Populate (with each iteration) with some entities...
        model->resetMeshCounters();
        /// ## Random new count
        const Qx::x_count min = .25 * cnt;
        const Qx::x_count max =  cnt;
        const Qx::x_count nwCnt = rand()%(max-min + 1) + min;

        for (int idx = 0; idx < nwCnt; ++idx) {
            ECSPosition pos;
            pos.x = rand() % 200 - 100;
            pos.y = rand() % 200 - 100;
            pos.z = rand() % 200 - 100;
    ...
    ecsWorld->system<Qx::MeshView, ECSPosition>("MovementSystem")
        .each([this](flecs::entity e, Qx::MeshView& mesh, ECSPosition &pos ) {
